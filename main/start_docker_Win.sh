...
sleep 15
winpty docker exec -ti rabbitmq2 bash -c "$JOIN_RABBIT2_RABBIT1"
winpty docker exec -ti rabbitmq3 bash -c "$JOIN_RABBIT3_RABBIT1"
winpty docker exec -ti rabbitmq4 bash -c "$JOIN_RABBIT4_RABBIT1"
winpty docker exec -ti rabbitmq1 bash -c "$OPTIONAL_COMMAND"